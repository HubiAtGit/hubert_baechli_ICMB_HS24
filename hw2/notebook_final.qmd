...
To save computation time, the number of agents is reduced from 5000 to 1000. Additionally, the population is sorted and assigned an ID to track them. Agents with small IDs receive the least money, while agents with large IDs have a lot of money.
...
At the beginning, the distribution of money in the population looks like this. After simulating 50,000 money exchanges, the following distribution emerges.
...
Fig05 <- figProb(calc_p(sNorm$Timeline[,4]), 
...
After 50,000 meetings, the distribution of probabilities in the population looks like this.
...
Fig10 <- figProb(calc_p(eNorm$Timeline[,2]), 
                "Probability after 500 exchanges", 850)
Fig11 <- figProb(calc_p(eNorm$Timeline[,4]), 
                "Probability after 1000 exchanges", 850)
...
After 50,000 meetings, the distribution of probabilities in this population looks almost identical.
...
This makes sense because the meetings are random, and therefore some agents keep their starting balance of 100 longer. After 1000 meetings, or about two meetings per agent, this becomes increasingly unlikely.
