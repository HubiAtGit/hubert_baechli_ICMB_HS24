...
hist(df1_even$Ms, xlab="Money",main="Histogram for split-random Restricted",
     col=rgb(0,0,1,0.2),
     xlim=c(0,max(c(df$Me,df$Ms)))*1.1,ylim=c(0,nrow(df)))
...
1.  you have to meet someone who already has over 1000 (p\~0.5)
...
**The real value should be under 50%**
...
I must have made something like a conjunction error here. the conditional probability for winning of 50% is only given if everyone has the same amount of money. otherwise the chance of winning is always smaller, which means that there are more and more poor and only a few rich agents in this game.
